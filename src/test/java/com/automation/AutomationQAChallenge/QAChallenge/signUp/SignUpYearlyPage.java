...
/**
 * This test case signs up for an yearly plan. It starts from the sign up window and completes the full flow
 * 
 */
public class SignUpYearlyPage extends CommonImpl{
	...
	//This test covers register user, buy yearly plan and payment by using credit card.
	@Test(timeout = 200000)
	public void signUPMonthly_Test() throws Exception {
		...
		Thread.sleep(5000);
		driver.findElement(By.cssSelector(selectorMap.get("YEARLY_PLAN"))).click();
		...
		verifyPageTitle("Delivery address", "PAGE_TITLE4");
		registerAddress();								//Registering the address 
		verifyAndClickAddressPopUp();					//verifying the popup window and the address on the popup wiindow
